...
```
```{r , fig.cap = "DJ LeMahieu (born July 13, 1988 in Visalia, California): Second Base, Colorado Rockies.  DJ's WAR rating is 1.4.", fig.height =2.0}
knitr::include_graphics("images/dj-lemahieu-2.jpg")
```
...
The Rox had a mixed but successful week.  They started the week away losing twice to the Pirates, winning against them in the last game of the series.  They started a home stand against the Giants on Thursday, winning three (maybe four).  Here is an interesting fact, **the Rockies have been first place in their division fifty-seven days out sixty-four days of the season** (counts all days of the season including off days).
...
The study of Sabermetrics is an evolving methodlogy for measuring and understanding baseball.  Since the eary work by Bill James, there has been an effort to create metrics that capture the worth or ranking of a player.  We have moved on from just stating a ballplayer's batting average to noting his effectiveness of getting base (OBP) and his ability to get extra-base hits (SLG).  And, of course, we combine OBP and SLG to represent (OPS) a player's overall effectiveness as a batter.

One of the more esoteric metrics you may hear about is WAR or Wins Above Replacement.  The theory is to calculate a number to express a players worth compared to all other players of a similar position.  How many games has this team won due to the contributions of player being measured, as compared to an average replacement.  

Let us use a Rockies player as an example.  Currently, Rockies third baseman, Nolan Arenado has a WAR rating of  2.9.  Nolan is currently the Rockies highest-rated WAR player.  A WAR of 2.9 is the number of games above what the team would expect with a replacement.
...
There is no agreed upon standard calculation for WAR statistics.  Three primary sources of WAR values are **Baseball Prospectus, Baseball Reference and FanGraphs.**  The Baseball Reference version of WAR is named **bWAR** or **rWAR.**  FanGraphs uses the abbreviation **fWAR** for their particular formulation of the WAR value.

WAR is not recognized as an official statistic by **Major League Baseball** or by the **Elias Sports Bureau.**  Although flawed and often miss-applied, WAR is ths most comprehensive statistic for measuring a player's value.

Next week, we will show some examples of fWAR values for position players and pitchers.  

Following is a table of Rockies players and their Baseball Reference **bWAR** values.

```{r}
Rockies_bWAR <-  tibble(Rank = c(1:12), Player = c("Kyle Freeland", "Nolan Arenado", "Charlie Blackmon", 
                     "Antonio Senzatela", "Tyler Chatwood", "Greg Holland", "Jake McGee", 
                     "Jeff Hoffman", "DJ LaMahieu", "Mark Reynolds", "German Marquez", "Chris Rusin"), 
                      bWAR = c(2.9, 2.8, 2.3, 2.1, 1.9, 1.5, 1.5, 1.4, 1.4, 1.2, 1.2, 1.1))
...
```{r WinLossMargin17,fig.height=3.0,fig.width=9.5}
...
```{r WinLossMargin07,fig.height=3.0, fig.width=9.5}
...
The Rockies current record is `r Current$Wins` Wins and `r Current$Losses` Losses.  The Rockies have scored `r TotalR` runs and have had `r TotalRA` runs scored against them.
