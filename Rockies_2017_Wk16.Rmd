...
header-includes:
  ...
  \pagestyle{fancy}
  \fancyhead[CO,CE]{}
  \fancyfoot[CO,CE]{Colorado Rockies Newsletter 2017, Week \#16}
...
```
```{r Headline,fig.cap = "Kyle Freeland breezes through the Pirates, Mark Reynolds homers twice in Rockies’ blowout win at Coors Field. July 23, 2017, Denver Post"}
knitr::include_graphics("images/kyle-freeland3.jpg")
```
...
It has been a better week for the Rockies, winning six of seven.  Three of the wins were by margins of nine or more runs.

We will continue our theme of WAR ratings and explore the Rockies contribution to the MLB 2017 All-Star game in terms of the attendees WAR values.

No fancy graphics just some special summaries of the data in table form.  I hope you find it informative.

Here is a synopsis of what is included in this week's newsletter.

* **Featured Article**  
    + MLB 2017 All-Star Game; How did Rockies Stack Up?
...
roster <- read_xlsx("./data/All_Star_Rosters.xlsx")
roster <- add_column(roster, WAR = 0.0)

# Lookup Player's WAR from Daily 
for (i in 1:nrow(roster)){
  if (grepl("P",roster$Pos[i])) {
    war <- filter(pitWar,name_common == roster$Player[i] & year_ID == 2017)$WAR
  }else {
    war <- filter(batWar,name_common == roster$Player[i] & year_ID == 2017)$WAR  
  }
  roster$WAR[i] <- war
}
...
Following is a table of all of the players invited to the All-Star game this year.  Several teams were represented by more than one player as you will see in subsequent tables.

```{r}
ros <- arrange(roster, League, Team)
knitr::kable(select(ros,c(1,5,2,3,4,7)), 
             cap="Player WAR Ratings")
```

## MLB All-Star Game: WAR Contribution by League

```{r}
League.WAR <- arrange(roster,League) %>% group_by(League) %>%
  summarize(Lg.WAR = sum(WAR))
knitr::kable(League.WAR, cap = "Total WAR by League")
```

\newpage

## MLB 2017 All-Star Game: WAR Contribution by Starters

Of the four Rockies All-Star players, two, Nolan Arenado and Charlie Blackmon, were starters.  The total of their WAR ratings placed the Rockies as third highest in the 2017 All-Star game.  Only the Houston Astros and the Washington Nationals ranked above them.

```{r}
Team_Starters.WAR <- filter(roster, Status == "Starter") %>% 
  arrange(Team.Short) %>% group_by(League, Team) %>%
  summarize(Players = n(), Total.WAR = sum(WAR)) %>% arrange(desc(Total.WAR))
knitr::kable(Team_Starters.WAR, 
      cap="Team Total WAR for Starters Only")
```

\newpage

## MLB 2017 All-Star Game: WAR Contribution by Team

The table that follows is a summary of the total WAR value of the players that attended.

```{r}
# Calculate the Total WAR contributed by each team.
Team.WAR <- arrange(roster,Team.Short) %>% group_by(League, Team) %>%
  summarize(Players = n(), WAR = sum(WAR)) %>% arrange(desc(WAR)) 
Team.WAR <-  ungroup(Team.WAR) %>% mutate(Rank = c(1:nrow(Team.WAR)))
knitr::kable(Team.WAR[,c(5,1,2,3,4)], 
         cap="Team Total WAR for all All-Star Players")
```
