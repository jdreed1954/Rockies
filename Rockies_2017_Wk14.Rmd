...
header-includes:
  ...
  \pagestyle{fancy}
  \fancyhead[CO,CE]{}
  \fancyfoot[CO,CE]{Colorado Rockies Newsletter 2017, Week \#14}
...
```
```{r Headline,fig.cap = "So, what is the significance of this picture of the Colorado Rockies?  Hint, do you like pi?", fig.height =2.0}
knitr::include_graphics("images/Rockies_Pi_Day.jpg")
```
...
So, we are over half way through the season at ninety-one games.  The Rockies have won fifty-two games.  Will they finish with over ninety wins?  The stats indicate they have a good chance of finishing aroud ninety wins.  Common wisdom is that a team must win at least ninety games to have a chance in the playoffs.

For this week's featured article, I pose the question, "Are **Team Payroll** and **Team Combined WAR** values correlated?"  In other words, if a team gets an extra $10 million for payroll, what, if any, improvement can the team expect in its Team Combined WAR rating?

Here is a synopsis of what is included in this week's newlstter.

* **Featured Article**  
    + Are Team Payroll and Team Combined WAR values correlated?
...
This week we will focus on the question of whether team payroll and combined team WAR rating are correlated.
...
WARrf <- select(WARrf,c(17,15))
combo_data <- merge(WARrf,Payroll) 
combo_data <- mutate(combo_data,Payroll = Pay17/1e6)
disp_data <- select(combo_data, c(1,2,4)) %>% arrange(desc(WAR))

```

## Exploring the Correlation Between Team Payroll and Combined WAR

Following is a table showing each teams total payroll and their current Team Combined WAR value from FanGraphs.  Looking at the data, can you tell if having a higher payroll yields a better WAR value?

Let me know what you think and next week we will explore this mathematically.  For now, though, give me the benefit of your intuition.  Note that the highest payroll team, the Los Angeles Dodgers have the highest WAR value.  Is this a coincidence?

More detail next week.

```{r}
knitr::kable(disp_data, digits = 2, caption = "MLB Teams Team Combined WAR and 2017 Payroll (Million)")
```

```{r CombinedWAR, message=FALSE, warning=FALSE}
